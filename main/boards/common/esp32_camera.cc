...
struct Gc0308VendorReg {
    uint16_t reg;
    uint8_t value;
};

// Exact Coglet GC0308 vendor profile derived from GC0308_480_122_42.ini.
// The original table uses 0x25=0x0f; keep it 0x00 here because this runs
// after VIDIOC_S_FMT and before VIDIOC_STREAMON. The sensor driver's
// S_STREAM path enables 0x25=0x0f when streaming starts.
static constexpr Gc0308VendorReg kCogletGc0308VendorProfile[] = {
    {0xfe, 0x00},
    {0x0f, 0x00},
    {0x01, 0x6a},
    {0x02, 0x70},
...
#ifdef CONFIG_BOARD_TYPE_BREAD_COMPACT_WIFI_CAM
    // Re-apply the exact known-good Coglet GC0308 vendor state while the
    // stream is still off. This keeps the project independent of edits under
    // managed_components while preserving the successful register sequence.
    if (!ApplyCogletGc0308VendorProfile()) {
        ESP_LOGE(TAG, "Failed to apply Coglet GC0308 vendor profile");
        close(video_fd_);
        video_fd_ = -1;
        sensor_format_ = 0;
        return;
...
#ifdef CONFIG_BOARD_TYPE_BREAD_COMPACT_WIFI_CAM
bool Esp32Camera::ApplyCogletGc0308VendorProfile() {
    esp_cam_sensor_id_t chip_id = {};
    if (!SensorPrivateIoctl(ESP_CAM_SENSOR_IOC_G_CHIP_ID,
                            &chip_id,
                            sizeof(chip_id),
                            false)) {
        ESP_LOGE(TAG, "Cannot read camera sensor ID before vendor profile");
        return false;
    }

    if (chip_id.pid != 0x9b) {
        ESP_LOGE(TAG,
                 "Coglet camera expects GC0308 PID 0x9b; detected PID=0x%lx",
                 static_cast<unsigned long>(chip_id.pid));
        return false;
    }

    for (const auto& reg : kCogletGc0308VendorProfile) {
        if (!WriteSensorRegister(reg.reg, reg.value)) {
            ESP_LOGE(TAG,
                     "GC0308 vendor profile write failed at reg 0x%02x=0x%02x",
                     static_cast<unsigned>(reg.reg),
    ...
    ESP_LOGI(TAG,
             "Applied Coglet GC0308 vendor profile (%u register writes)",
             static_cast<unsigned>(sizeof(kCogletGc0308VendorProfile) /
                                    sizeof(kCogletGc0308VendorProfile[0])));
    return true;
}
#endif
